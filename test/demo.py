...
for n_iter in range(100):
    ...
    writer.add_scalars('data/scalar_group', {"train": n_iter * np.cos(n_iter)}, n_iter)
    writer.add_scalars('data/scalar_group', {"test": n_iter * np.sin(n_iter)}, n_iter)
    ...
    if n_iter % 10 == 0:
        ...
        for i in range(x.size(0)):
            x[i] = np.cos(
                freqs[n_iter // 10] * np.pi * float(i) / float(sample_rate))  # sound amplitude should in [-1, 1]
